...
VibeBar は、**Claude Code**・**Codex**・**OpenCode** の TUI セッション状態をリアルタイムで監視できる、軽量な macOS メニューバーアプリです。
...
- `vibebar` ラッパーは `claude` / `opencode` にも対応していますが、この 2 つはプラグイン連携が優先されます。
...
- Claude/OpenCode のプラグイン管理（インストール・アンインストール・更新）をアプリ内で完結。
...
- `plugins/*`：Claude/OpenCode プラグインパッケージ。
...
4. ラッパー経由で Codex を起動（推奨）：

```bash
swift run vibebar codex -- --model gpt-5-codex
```

5. フォールバック：プラグインが使えない場合、ラッパー経由で Claude/OpenCode を起動：
