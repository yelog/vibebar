...
- 노치가 있는 MacBook 화면에서는 선택적으로 노치 표시 모드를 사용할 수 있으며, 노치 오른쪽에 작은 검은 아이콘 영역을 연장 표시합니다. 현재 주 디스플레이가 지원하지 않으면 일반 메뉴 막대 진입점으로 자동 복귀합니다.
...
VibeBar는 지원되는 AI 도구 간의 토큰 사용량을 추적하고 자세한 분석과 시각화를 제공합니다:

**지원되는 도구:**
- **Claude Code** — `~/.config/claude/projects/*/usage.jsonl`에서 읽기
- **Codex** — `~/.codex/sessions/*/usage.jsonl`에서 읽기
- **OpenCode** — `~/.local/share/opencode/opencode.db`에서 읽기
...
**시각화 옵션:**
- **GitHub 스타일 히트맵** — 색상 강도로 사용량을 표시하는 39주 활동 매트릭스
- **막대 그래프** — 시간대별 누적 막대 그래프
- **선 그래프** — 사용량 추세를 보여주는 선 그래프

**구성 옵션:**
- **토큰** 또는 **비용** 보기 전환
- 세분성 조정: 시간 / 일 / 주 / 월
- 그룹화: 도구 / 모델 / 없음
- 새로고침 간격 설정: 5분 / 15분 / 30분 / 1시간
- 표시할 최대 시리즈 수 사용자 지정

메뉴 바 드롭다운에서 AI 사용 패턴과 비용 개요를 한눈에 확인할 수 있습니다.
